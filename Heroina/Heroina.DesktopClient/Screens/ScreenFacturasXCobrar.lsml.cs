...
{
    public partial class ScreenFacturasXCobrar
    {
        #region Propiedades
        ...
        partial void ScreenFacturasXCobrar_InitializeDataWorkspace(List<IDataService> saveChangesTo)
        {
            InitDataWorkspace();
            FindControls();
            Bindings();
        ...
        partial void Empresas_Loaded(bool succeeded)
        {
            comboEmpresas = this.FindControl("ComboEmpresas");
            if (Cfg != null)
                Empresas.SelectedItem = Empresas.Where(e => e.Nombre == Cfg.Empresa).FirstOrDefault();

#if DEBUG
            comboEmpresas.IsEnabled = Empresas.Count > 1;
#else
                // Seguridad
                comboEmpresas.IsEnabled = Empresas.Count > 1 && Application.Current.User.HasPermission("CambiarEmpresa");
#endif
        }
